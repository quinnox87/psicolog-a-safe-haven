...
const ServiceCard: React.FC<ServiceCardProps> = ({ service, variant = 'preview' }) => {
  ...
  return (
    <div 
      className={`service-card bg-white rounded-2xl border border-[#e5e7eb] shadow-[0_4px_20px_rgba(0,0,0,0.03)] transition-all duration-300 ${
        isPreview 
    ...
    >
      <div 
        className={`service-card__icon p-5 rounded-full flex items-center justify-center shrink-0 ${
          service.id === 'neuro' ? 'bg-primary/10 text-primary' : 'bg-accent-blue/10 text-accent-blue'
